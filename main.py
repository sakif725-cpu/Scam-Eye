...
def analyze_voice(data: VoiceDataRequest):
    ...
    if data.text and data.text.strip():
        ...
        print("\n" + "="*60)
        print(f"📥 [RAW TEXT RECEIVED FROM APP]: \"{text}\"")
        print("="*60 + "\n")
        logger.info("Processing direct text analysis: \"%s\"", text)
        risk_result = None
        try:
            from src.predictor import predict_message
            risk_result = predict_message(text)
            is_scam = risk_result.get("prediction") == "FRAUD"
            print(f"🛡️ [AI MODEL PREDICTION] -> Scammer: {is_scam} | Risk: {risk_result.get('risk_score', 0)} | Category: {risk_result.get('threat_category', 'General')}")
        ...
        if risk_result:
            ...
            return VoiceAnalysisResponse(
                isBot=False,
                isScammer=is_scam,
                confidence=round(float(risk_result.get("confidence", 0.9)), 2),
                riskScore=int(float(risk_result.get("risk_score", 0.0)) * 100),
                riskLevel=risk_result.get("risk_level", "LOW"),
                intention=risk_result.get("threat_category", "Suspicious Conversation" if is_scam else "💬 Natural Human Conversation"),
                summary=risk_result.get("reason", "Live speech analyzed."),
                behaviorSummary=risk_result.get("reason", "Live speech analyzed."),
                ...
                scamType=risk_result.get("threat_category") if is_scam else None,
                transcript=text
            )
    ...
    # 2. Audio Transcript evaluation
    transcript = data.text.strip() if (data and data.text) else ""
    if not transcript and audio_bytes:
        try:
            ...
            if transcript:
                print("\n" + "="*60)
                print(f"🎙️ [AUDIO TRANSCRIBED TO TEXT]: \"{transcript}\"")
                print("="*60 + "\n")
                logger.info("Transcribed Audio: \"%s\"", transcript)
        except Exception as e:
            logger.debug("Audio transcription step: %s", e)
